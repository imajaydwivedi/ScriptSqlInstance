...
#$resultGetReplArticles = Invoke-DbaQuery -SqlInstance $Distributor -Database $DistributionDb -Query $tsqlGetReplState

foreach($article_row in ($resultGetReplArticles | Select-Object -Property publisher, publisher_db, source_owner, article -Unique))
{
    $article_row
    break;
}
